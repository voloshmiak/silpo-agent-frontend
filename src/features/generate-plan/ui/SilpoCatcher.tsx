...
type FallingItem = { id: number; x: number; y: number; good: boolean; emoji: string };
...
export const SilpoCatcher: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const spawn = () => {
      ...
      const products = good ? GOOD_PRODUCTS : BAD_PRODUCTS;
    ...
    const tick = (time: number) => {
      ...
      const playerLeft = (playerRef.current / 100) * FIELD_WIDTH - PLAYER_WIDTH / 2;
      ...
      for (const item of items) {
        ...
        const itemLeft = (item.x / 100) * FIELD_WIDTH - ITEM_SIZE / 2;
        ...
        if (caught) {
          if (item.good) { currentScore += 10; setScore(currentScore); }
          else { currentLives -= 1; setLives(currentLives); if (currentLives <= 0) { stopped = true; setGameOver(true); } }
  ...
  const restart = () => {
    ...
    setPlayerX(50); setScore(0); setLives(3); setGameOver(false); setFallingItems([]); setStarted(true);
    ...
    <div className="space-y-3">
      ...
      <div ref={fieldRef} className="relative h-[220px] w-full overflow-hidden rounded-xl border-2 border-[#D8D2C2] bg-[#F4F1E8] touch-none select-none" aria-label="Гра Silpo Catcher. Рухайте тележку стрілками або пальцем">
        ...
        {!started && <div className="absolute inset-0 z-20 flex flex-col items-center justify-center gap-3 bg-[#ECE8DC]/95 px-6">
          ...
          <p className="max-w-[230px] text-xs leading-relaxed text-zinc-500">Лови броколі, авокадо, гречку, курочку та знижки: вони дають +10 балів. Пончики, кола й чипси забирають життя.</p>
